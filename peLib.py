...
class AdjustPE:
   ...
   # Adjust IMAGE_FILE_HEADER
    ...
    def _set_jmp_text_sec(self, moving_regions, DEBUG=False):
        """
        Wipe out the displaced regions and set jmp instructions in a .text section
        """
        imageBase = self.peinfo.getImageBase()
        textRva = self.peinfo.getRVA(0)

        # Caution! This should be pointer to .text section, not RVA!!
        text_section_offset = self.peinfo.getPointerToRawData(0)

        if DEBUG:
            print "imageBase:0x%08X, textRVA:0x%08X, textSectionOffset:0x%08X"\
                  % (imageBase, textRva, text_section_offset)

        for (mov_start, mov_size, ropf_start) in moving_regions:
            offset_in_text = mov_start - imageBase - textRva

            # Wipe out the displaced region with 0xCC (int 3) in a text section
            self.pe.set_bytes_at_offset(text_section_offset + offset_in_text, '\xCC' * mov_size)

            # Set jmp [jmp_to_rel_addr] ex) '\xE9\xaa\xbb\xcc\xdd'
            jmp_code = struct.pack('<B', 0xE9) + struct.pack('<i', ropf_start - mov_start - 5)
            self.pe.set_bytes_at_offset(text_section_offset + offset_in_text, jmp_code)

            if DEBUG:
                print '\t[0x%08X:0x%08X] <-> [0x%08X:0x%08X]' % (mov_start, mov_start + mov_size, ropf_start, ropf_start + mov_size + 5)
                print "Set jmp 0x%08X in the .text section" % (text_section_offset + offset_in_text)

    def _set_transformations(self, selected_diffs):
        base = self.peinfo.getImageBase()
        for rva, old, new in list(itertools.chain(*selected_diffs)):
            if not self.pe.set_bytes_at_rva(rva - base, new):
                print "\tError setting bytes for transformation!"
    ...
    def update_section(self, moving_regions, moving_code, reloc_entries, selected_diffs, DEBUG=False):
        """
        ...
        # Fill out the displaced regions with 0xCC and set jmp instructions in a .text section
        self._set_jmp_text_sec(moving_regions)

        # Set out bytes with randomization (IPR) techniques
        self._set_transformations(selected_diffs)
